    ...
    targets.map(async (lang) => {
      const r = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.2,
          messages: [
            { role: 'user', content: `Translate into ${lang}. Return only the translation:\n\n${englishText}` },
          ],
        }),
      });
      if (!r.ok) return [lang, ''];
  ...
  try {
    ...
    try {
      ...
      addLine(code, {
        ...
        tx: Object.fromEntries(targets.map((l) => [l, ''])),
      });
      return NextResponse.json({ ok: false, error: 'transcription_failed' }, { status: 502 });
    }

    if (!englishText) return NextResponse.json({ ok: true, empty: true });
